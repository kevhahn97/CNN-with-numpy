...
def convolution2d(x, kernel, stride):
    ...
    conv_out = None
    # =============================== EDIT HERE ===============================
    conv_out_size = ((height - kernel_size) // stride) + 1
    conv_out = np.zeros((conv_out_size, conv_out_size))
    for krow in range(conv_out_size):
        for kcol in range(conv_out_size):
            conv_out[krow, kcol] = np.sum(np.multiply(x[krow*stride:krow*stride+kernel_size, kcol*stride:kcol*stride+kernel_size], kernel))
    # =========================================================================
    return conv_out
...
class ReLU:
    ...
    def backward(self, d_prev):
        ...
        dz = None
        # =============================== EDIT HERE ===============================
        dReLU = np.ones_like(d_prev)
        dReLU[self.zero_mask] = 0
        dReLU[np.logical_not(self.zero_mask)] = 1
        dz = dReLU * d_prev
        # =========================================================================
        return dz
...
class ConvolutionLayer:
    ...
    def convolution(self, x, kernel, bias=None, stride=1, pad=0):
        ...
        conv = None
        # =============================== EDIT HERE ===============================
        # using convolution2d implementation (which also works)
        """conv_out_size = ((height - kernel_size) // stride) + 1
        conv = np.zeros((batch_size, out_channel, conv_out_size, conv_out_size))
        for batch in range(batch_size):
            for idx_out in range(out_channel):
                list_conv_of_input = list()
                for idx_in in range(in_channel):
                    list_conv_of_input.append(convolution2d(x[batch, idx_in], kernel[idx_out, idx_in], stride))
                conv[batch, idx_out] = sum(list_conv_of_input)
                if bias is not None:
                    conv[batch, idx_out] += bias[idx_out]"""

        # numpy implementation (for faster training)
        conv_out_size = ((height - kernel_size) // stride) + 1
        target = np.zeros((batch_size, in_channel, conv_out_size, conv_out_size, kernel_size, kernel_size), dtype=np.float64)
        for krow in range(conv_out_size):
            for kcol in range(conv_out_size):
                target[:, :, krow, kcol, :, :] = x[:, :, krow * stride:krow * stride + kernel_size, kcol * stride:kcol * stride + kernel_size]
        target = target.transpose((0,2,3,1,4,5)).reshape((batch_size * conv_out_size * conv_out_size, in_channel * kernel_size * kernel_size))
        kernel = kernel.reshape(out_channel, -1).T
        if bias is not None:
            conv = np.matmul(target, kernel).reshape(batch_size, conv_out_size,
                                                     conv_out_size, out_channel).transpose(0, 3, 1, 2) \
                   + bias.reshape(1, bias.shape[0], 1, 1)
        else:
            conv = np.matmul(target, kernel).reshape(batch_size, conv_out_size,
                                                     conv_out_size, out_channel).transpose(0, 3, 1, 2)
        # =========================================================================
    ...
    def backward(self, d_prev):
        ...
        ** [HINT] **
        ...
        # =============================== EDIT HERE ===============================
        # dW
        self.dW = self.convolution(self.zero_pad(self.x, self.pad).transpose(1,0,2,3),
                                   d_prev.transpose(1,0,2,3)).transpose(1,0,2,3)

        # db
        self.db = np.sum(d_prev, axis=(0, 2, 3))

        # dx
        dx = self.convolution(self.zero_pad(d_prev, kernel_size-1),
                              np.flip(self.W, axis=(2,3)).transpose(1,0,2,3),
                              bias=None)[:, :, self.pad:self.pad+height, self.pad:self.pad+width]
        # =========================================================================
        return dx
    ...
    def zero_pad(self, x, pad):
        ...
        # =============================== EDIT HERE ===============================
        padded_x = np.zeros((batch_size, in_channel, height + pad * 2, width + pad * 2))
...
class MaxPoolingLayer:
    ...
    def forward(self, x):
        ...
        """
        pool_out_size = ((height - self.kernel_size) // self.stride) + 1
        max_pool = np.zeros((batch_size, channel, pool_out_size, pool_out_size))
        for batch in range(batch_size):
            for ch in range(channel):
                for pout_row in range(pool_out_size):
                    for pout_col in range(pool_out_size):
                        row_start = pout_row*self.stride
                        col_start = pout_col*self.stride
                        max_idx = np.argmax(x[batch, ch, row_start:row_start+self.kernel_size, col_start:col_start+self.kernel_size])
                        max_pool[batch, ch, pout_row, pout_col] = x[batch, ch, row_start + max_idx//self.kernel_size, col_start + max_idx%self.kernel_size]
                        self.mask[batch, ch, row_start + max_idx // self.kernel_size, col_start + max_idx % self.kernel_size] = 1
        """

        # numpy implementation (faster)
        pool_out_size = ((height - self.kernel_size) // self.stride) + 1
        target = np.zeros((batch_size, channel, pool_out_size, pool_out_size, self.kernel_size, self.kernel_size))
        for pout_row in range(pool_out_size):
            for pout_col in range(pool_out_size):
                target[:, :, pout_row, pout_col, :, :] = x[:, :, pout_row * self.stride:pout_row * self.stride + self.kernel_size,
                                                 pout_col * self.stride:pout_col * self.stride + self.kernel_size]
        target = target.reshape(
            (batch_size * channel * pool_out_size * pool_out_size, self.kernel_size * self.kernel_size))
        max_idx = np.argmax(target,axis=1)
        max_pool = target[range(max_idx.size), max_idx].reshape(batch_size, channel, pool_out_size, pool_out_size)
        max_idx = max_idx.reshape(batch_size, channel, pool_out_size, pool_out_size)
            ...
            for ch in range(channel):
                for pout_row in range(pool_out_size):
                    for pout_col in range(pool_out_size):
                        row_start = pout_row * self.stride
                        col_start = pout_col * self.stride
                        self.mask[batch, ch, row_start + max_idx[batch, ch, pout_row, pout_col]//self.kernel_size, col_start + max_idx[batch, ch, pout_row, pout_col]%self.kernel_size] = 1
        # =========================================================================
    ...
    def backward(self, d_prev=1):
        ...
        for b in range(batch):
            for ch in range(channel):
                for pout_row in range(height):
                    for pout_col in range(width):
                        row_start = pout_row * self.stride
                        col_start = pout_col * self.stride
                        d_max[b, ch, row_start:row_start + self.kernel_size, col_start:col_start + self.kernel_size] \
                            = self.mask[b, ch, row_start:row_start + self.kernel_size,
                              col_start:col_start + self.kernel_size] * d_prev[b, ch, pout_row, pout_col]
        # =========================================================================
        return d_max
...
class FCLayer:
    ...
    def backward(self, d_prev):
        ...
        self.db = np.matmul(np.ones(self.x.shape[0]).T, d_prev)
...
class SoftmaxLayer:
    ...
    def backward(self, d_prev=1):
        ...
        dx = None
        # =============================== EDIT HERE ===============================
        dx = d_prev * (self.y_hat - self.y) / batch_size
        # =========================================================================
        return dx
    ...
    def ce_loss(self, y_hat, y):
        ...
        self.y = y
        # =============================== EDIT HERE ===============================
        self.loss = np.mean(-1 * np.log(np.sum((y_hat + eps) * y, axis=1)))
        # =========================================================================
        return self.loss
